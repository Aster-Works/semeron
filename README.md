...
**Semeron** は、教会員が毎日 **みことば → 牧師の短い導き → 黙想 → 祈り → Read/Prayed → 今日の祈り → 今日の応答** の順に向かうための、教会単位の **デボーション・祈りリズム PWA** です。総合教会アプリでも、Christian SNS でも、AI デボーション玩具でもありません。将来の `Aster Church` に統合される `Daily / Rhythm` モジュールとして設計しています。

現在は **実 Supabase 接続済みのパイロット版** です。メール+パスワード認証、教会作成/招待参加、デボーションCRUD、祈祷課題の投稿/承認、Todayの応答、Web Push/in-app通知、PWA、Pastor Assistの人間レビュー前提UIまで含みます。

設計・実装メモ: `JimiVault/02_Area/個人/Aster Works/Semeron/`。
...
```bash
npm run db:start       # Supabaseローカルスタック起動
npm run db:test        # 現在のローカルDBに対するpgTAP
npm run db:test:reset  # db:reset後にpgTAP。リリース前の推奨ゲート
```
...
- **Vitest / Playwright / pgTAP**（unit、PWA/会員導線E2E、RLS）

## ローカルの歩き方

- 入口は **マーケLPではなく「今日のみことば」**（`/ja/church/eifuku-minami/today`）。
- ローカルDBを起動/リセットすると、seedユーザーは全員 `password123` でログインできます。
- 代表アカウント:
  - `jimi@eifuku.example` = 永福南の牧師/owner
  - `taro@eifuku.example` = 永福南の一般会員
  - `ken@eifuku.example` = 祈祷チーム
...
**Pastor Assist（AI）は管理者限定・教会ごとのopt-in**です。AIは下書きと提案だけを返し、保存・承認・公開は必ず人間の別操作です。

## セキュリティ・プライバシー設計

祈祷課題は要配慮情報（健康・家庭・経済・信仰・未成年）を含み得るため、DB側は **Supabase RLS + private認可ヘルパー** を正とし、画面側は補助的な表示制御を重ねています。匿名投稿は `content_feed` 経由で作者をマスクし、完了ログは本人だけに見せ、管理者には匿名集計だけを返します。
...
- Pastor Assistの実生成には `ANTHROPIC_API_KEY` が必要です。未設定時はUI上でgracefulに無効化されます。
- OS Pushの実配信にはVAPID鍵と、iOSではホーム画面追加/iOS16.4+が必要です。in-app通知は常にフォールバックとして残ります。
- ローカルDBを手動操作した後のRLS検証は、`npm run db:test:reset` でseedから作り直して確認するのが推奨です。

## 実装履歴

以下はパイロットまでの実装フェーズ記録です。現在の正は、このREADME冒頭のクイックスタート/運用情報と最新コードです。

## Phase 2 — Supabase スキーマ + RLS（ローカル）

DB 層の「安全設計」を実コードに落としたフェーズ。`supabase/` にローカル移行とRLSテストを用意しました。
...
npm run db:test    # pgTAP テスト（= supabase test db）
npm run db:test:reset # db:reset後にpgTAP。汚れたローカルDBを避けるリリース前推奨ゲート
...
- 可視性は `private.can_view_content()` に一元化（04 §4/§5 を DB 側で実装。初期UIの `visibility.ts` と同じ規則）。
...
> Phase 2 の SQL は敵対的レビュー（3次元）で検証し、2件の HIGH（自己公開/自己承認の抜け穴・越境整合性）を含む指摘を修正済み。RLSは `npm run db:test:reset` で seed から作り直した状態をリリース前ゲートとして確認します（教会分離・承認制・匿名マスク・完了ログ本人限定・可視性マトリクス・admin≠moderator・祈祷チーム権限境界ほか）。生成型も取得済み。
...
- **実機E2E検証済**: ログイン→自教会 Today（実データ・匿名マスク）→「祈りました」が永続。lint / typecheck / build / unit / RLS をリリース前ゲートとして確認。
...
- **トリガー**: 承認→`prayer_request_approved` / 却下→`prayer_request_rejected` / 祈られた→`prayer_request_prayed` / デボーション公開→閲覧可能な会員へ `daily_devotion_published`。SQL で各生成をロールバック確認。RLSテストで維持。
...
- **quality gate**: lint / typecheck / build（`/api/notifications/dispatch` を含む）/ unit / RLS。
...
- **quality gate**: lint / typecheck / build / unit（Pastor Assist の redaction・JSON 抽出を含む）/ RLS。
